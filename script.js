...
async function wwOpenResourceInApp(sid,rid){
  ...
  try{
    ...
    if(window.wwOpenFileInReader) window.wwOpenFileInReader(file,r.title||file.name,{resourceId:r.id,subjectId:sid,topicId:r.topic_id||null,name:r.title||file.name});
...
state.studyScope=state.studyScope||{subjects:{},topics:{}};state.tutorSession=state.tutorSession||null;state.problemSolver=state.problemSolver||null;state.tutorSessions=Array.isArray(state.tutorSessions)?state.tutorSessions:[];state.tutorMemory=state.tutorMemory||{version:'65.19',updatedAt:null,turns:0,facts:[],concepts:{},errorPatterns:{},preferences:{support:null},recent:[]};state.studentModel=state.studentModel||{version:'65.19',updatedAt:null,global:{attempts:0,correct:0,partial:0,incorrect:0,hints:0,avgConfidence:0,avgLatency:0,independence:0},topics:{},recent:[]};state.chatbotHistory=state.chatbotHistory||[];state.chatbotPdfContext=state.chatbotPdfContext||null;state.documentIntelligence=state.documentIntelligence||{resources:{},activeResourceId:null};state.sectionTutor=state.sectionTutor||null;
...
function wwTutorPrerequisite(topicId){return window.WWPrerequisite&&window.WWPrerequisite.plan?window.WWPrerequisite.plan(state,topicId):null}
    ...
    '<div class="header"><h1 class="app-brand"><img class="app-logo" src="logo.svg" alt="White Wolf Scholar"> <span class="app-brand-text">White Wolf</span></h1>'+
      '<div style="position:relative;display:flex;align-items:center;gap:8px;">'+
        ...
        '<div class="settings-dropdown" id="settings-menu">'+
          ...
          '<div class="section-label">IA & Tutor</div><div class="item" data-modal="ai-settings"><span class="icon">🧠</span> IA Tutor</div><div class="divider"></div><div class="section-label">Actions</div>'+
...
function renderStats(){
  ...
  var tabsHTML='<div class="inner-tabs">'+
    ...
    '<div class="inner-tab '+(tab==='tutor-sessions'?'active':'')+'" data-stats-tab="tutor-sessions">🧑‍🏫 Sessions</div>'+
  '</div>';
  var content='';
  ...
  else if(tab==='tutor-sessions')content=renderTutorSessionsScreen();
...
function renderTutorSessionsScreen(){
  var sessions=(Array.isArray(state.tutorSessions)?state.tutorSessions:[]).slice();
  var active=state.tutorSession&&state.tutorSession.active?state.tutorSession:null;
  var total=sessions.length,mins=sessions.reduce(function(a,x){return a+Number(x.durationMinutes||0)},0),attempts=sessions.reduce(function(a,x){return a+Number(x.attempts||0)},0),correct=sessions.reduce(function(a,x){return a+Number(x.correct||0)},0);
  var rows=sessions.map(function(x){var pct=x.attempts?Math.round(Number(x.correct||0)/Number(x.attempts)*100):0;return '<button class="ww-tutor-session-row" data-tutor-replay="'+wwEscapeHTML(x.id||'')+'"><div class="ww-tutor-session-main"><strong>🧑‍🏫 '+wwEscapeHTML(x.topicTitle||'Session Tutor')+'</strong><small>'+new Date(x.startedAt||Date.now()).toLocaleDateString('fr-FR')+' · '+wwEscapeHTML(x.mode||'socratic')+' · '+Number(x.durationMinutes||0)+' min</small></div><div class="ww-tutor-session-score">'+pct+'%<span>'+Number(x.attempts||0)+' tent.</span></div><span>›</span></button>'}).join('');
  var profile=window.WWTutorSession&&window.WWTutorSession.learningProfile?window.WWTutorSession.learningProfile(state):{accuracy:0,independence:0,strengths:[],needsWork:[]};
  var profileHTML='<div class="card ww-tutor-profile"><div class="card-title">🧠 Profil d’apprentissage</div><div class="ww-tutor-profile-kpis"><span><b>'+profile.accuracy+'%</b> précision</span><span><b>'+Math.round(Number(profile.independence||0))+'%</b> autonomie</span><span><b>'+Number(profile.attempts||0)+'</b> tentatives</span></div>'+(profile.strengths.length?'<p class="text-small">💪 Forces : '+profile.strengths.map(function(x){return wwEscapeHTML(x.title)}).join(' · ')+'</p>':'')+(profile.needsWork.length?'<p class="text-small">🎯 À renforcer : '+profile.needsWork.map(function(x){return wwEscapeHTML(x.title)}).join(' · ')+'</p>':'')+'</div>';
  return '<div class="ww-tutor-sessions">'+profileHTML+'<div class="card ww-tutor-session-hero"><div class="card-title">🧑‍🏫 Historique du Tutor</div><p class="text-muted text-small">Chaque session terminée devient une trace pédagogique consultable. Le replay est en lecture seule.</p><div class="ww-tutor-session-kpis"><span><b>'+total+'</b> sessions</span><span><b>'+mins+'</b> min</span><span><b>'+attempts+'</b> tentatives</span><span><b>'+correct+'</b> correctes</span></div>'+(active?'<div class="ww-tutor-active-note">🟢 Une session est actuellement active · '+wwEscapeHTML(active.topicTitle||'')+'</div>':'')+'</div><div class="card"><div class="card-title">📚 Sessions récentes</div>'+(rows||'<div class="text-muted text-small">Aucune session terminée pour le moment.</div>')+'</div></div>';
}

function renderTutorSessionReplay(id){
  var x=(state.tutorSessions||[]).find(function(s){return s.id===id}); if(!x)return '<div class="modal-overlay"><div class="modal-content"><h3>Session introuvable</h3><button class="btn-primary" data-close-modal>Fermer</button></div></div>';
  var pct=x.attempts?Math.round(Number(x.correct||0)/Number(x.attempts)*100):0;
  var actions=(x.actions||[]).map(function(a,i){return '<div class="ww-tutor-replay-action"><b>'+(i+1)+'. '+wwEscapeHTML(a.action||'Action')+'</b><span>'+wwEscapeHTML(a.reason||'')+'</span></div>'}).join('');
  var ev=x.lastEvaluation||{};
  return '<div class="modal-overlay"><div class="modal-content ww-tutor-replay-modal"><span class="close-btn" data-close-modal>❌</span><div class="ww-tutor-replay-kicker">WHITE WOLF / TUTOR REPLAY</div><h3>🧑‍🏫 '+wwEscapeHTML(x.topicTitle||'Session Tutor')+'</h3><p class="text-muted">'+new Date(x.startedAt||Date.now()).toLocaleString('fr-FR')+' · '+Number(x.durationMinutes||0)+' min · '+wwEscapeHTML(x.mode||'socratic')+'</p><div class="ww-tutor-replay-grid"><div><b>'+pct+'%</b><span>Score</span></div><div><b>'+Number(x.attempts||0)+'</b><span>Tentatives</span></div><div><b>'+Number(x.hints||0)+'</b><span>Indices</span></div><div><b>'+Number(x.incorrect||0)+'</b><span>Erreurs</span></div></div><div class="ww-tutor-replay-section"><strong>🎯 Objectif</strong><p>'+wwEscapeHTML(x.goal||'')+'</p></div><div class="ww-tutor-replay-section"><strong>🧠 Dernière évaluation</strong><pre>'+wwEscapeHTML(JSON.stringify(ev,null,2))+'</pre></div>'+(actions?'<div class="ww-tutor-replay-section"><strong>⚙️ Parcours du Tutor</strong>'+actions+'</div>':'')+'<div class="ww-tutor-replay-section"><strong>📌 Résultat</strong><p>'+wwEscapeHTML((x.summary&&x.summary.nextAction)||'—')+'</p></div><div class="modal-actions"><button class="btn-primary" data-close-modal>Fermer</button></div></div></div>';
}
...
function renderModal(){
  ...
  if(m.type==='ai-settings'){
    var cfg=window.WWAI&&window.WWAI.load?window.WWAI.load():{enabled:false,provider:'gemini',model:'gemini-2.5-flash',apiKey:''};
    return `<div class="modal-overlay"><div class="modal-content ww-ai-settings-modal"><span class="close-btn" data-close-modal>❌</span><h3>🧠 IA Tutor — cerveau du tuteur</h3><p class="text-muted text-small">La clé reste sur cet appareil dans le stockage local du navigateur. Ne publie jamais ta clé dans GitHub.</p><div style="display:grid;gap:12px;"><label class="ww-ai-toggle"><input id="ww-ai-enabled" type="checkbox" ${cfg.enabled?'checked':''}> <span>Activer le cerveau IA</span></label><label>Provider<select id="ww-ai-provider"><option value="gemini">Google Gemini</option></select></label><label>Modèle<select id="ww-ai-model"><option value="gemini-2.5-flash">Gemini 2.5 Flash</option><option value="gemini-2.5-flash-lite">Gemini 2.5 Flash-Lite</option><option value="gemini-2.5-pro">Gemini 2.5 Pro</option></select></label><label>Clé API Gemini<input id="ww-ai-key" type="password" autocomplete="off" placeholder="AIza..." value="${wwEscapeHTML(cfg.apiKey||'')}"></label><div class="text-muted text-small">Le modèle reçoit le périmètre actif, le chapitre, la maîtrise, les erreurs, les sessions et, si disponible, le texte de la section/document étudié.</div></div><div class="modal-actions"><button class="btn-outline" data-ai-reset-brain>🧠 Réinitialiser le suivi</button><button class="btn-outline" data-ai-clear>🗑️ Effacer la clé</button><button class="btn-primary" data-save-ai>💾 Enregistrer</button></div></div></div>`;
  }
...
function attachAppEvents(){
  ...
  document.querySelectorAll('[data-ai-reset-brain]').forEach(function(el){el.onclick=function(){state.chatbotTutorBrain={mode:'tutor',turns:0,lastTopicId:null,lastActivity:null,pendingQuestion:null,pendingStartedAt:null,hintUsed:false};if(window.WWStudentModel)window.WWStudentModel.reset(state);if(window.WWTutorMemory)window.WWTutorMemory.reset(state);saveState();showToast('🧠 Suivi pédagogique réinitialisé');render()}});
  document.querySelectorAll('[data-save-ai]').forEach(function(el){el.onclick=function(){var cfg={enabled:!!document.getElementById('ww-ai-enabled').checked,provider:document.getElementById('ww-ai-provider').value,model:document.getElementById('ww-ai-model').value,apiKey:document.getElementById('ww-ai-key').value.trim()};if(!cfg.apiKey)cfg.enabled=false;if(window.WWAI)window.WWAI.save(cfg);state.modal=null;showToast(cfg.enabled?'🧠 IA Tutor activée':'🧠 IA Tutor enregistrée');render()}});
  document.querySelectorAll('[data-ai-clear]').forEach(function(el){el.onclick=function(){if(window.WWAI)window.WWAI.clear();state.modal=null;showToast('🗑️ Clé IA supprimée de cet appareil');render()}});
  ...
  document.querySelectorAll('[data-tutor-replay]').forEach(function(el){el.onclick=function(e){e.stopPropagation();state.modal={type:'tutorReplay',id:this.dataset.tutorReplay};render()}});
...
function wwChatFormat(text){return String(text||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>').replace(/\n/g,'<br>')}
var wwChatbot=window.WWChatbot?WWChatbot.create({messages:chatMsgs,input:chatInput,state:state,format:wwChatFormat,upgrade:wwUpgradeIcons,save:saveState}):null;
