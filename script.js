...
// V61.8 Dependency Container — application code consumes stable service contracts.
...
function addError(desc,subjectId,cause,correction,difficulty){var err={id:'err_'+generateId(),description:desc,subject_id:subjectId||null,cause:cause||'other',correction:correction||'',difficulty:difficulty||'medium',status:'to_review',revisions:0,max_revisions:3,created_at:wwLocalDateISO(new Date()),last_reviewed:null,next_review:wwLocalDateISO(new Date())};state.errors.push(err);scheduleNextReview(err);saveState();return err}
function reviewError(id,success){var err=(state.errors||[]).find(function(e){return e.id===id});if(!err)return;err.last_reviewed=wwLocalDateISO(new Date());if(success){err.revisions++;if(err.revisions>=err.max_revisions)err.status='mastered';else err.status='in_progress'}else{err.revisions=0;err.status='to_review'}scheduleNextReview(err);saveState()}
...
// ============================================================
// WHITE WOLF V50 — INTELLIGENCE 2.0
// Additive dashboard layer: Planning → Mission → Study → Errors → Exams.
// It recommends; it never rewrites the user's Planning Hebdo.
// ============================================================
...
function wwIntelWeekSessions(){var now=new Date(),start=new Date(now);start.setHours(0,0,0,0);start.setDate(start.getDate()-6);var min=start.getTime();return (state.sessions||[]).filter(function(x){var d=new Date((x.date||'')+'T12:00');return isFinite(d.getTime())&&d.getTime()>=min&&d.getTime()<=Date.now()})}
...
function wwIntelWeakTopic(){var best=null,score=-1;state.topics.forEach(function(t){var p=getProgress(t.id),days=p.last_studied?Math.floor((Date.now()-new Date(p.last_studied).getTime())/86400000):99;var v=(4-p.level)*6+Math.min(10,Math.max(0,days*.5));if(v>score){score=v;best={topic:t,progress:p,days:days}}});return best}
function wwIntelSnapshot(){var mission=wwIntelMission(),today=wwIntelTodaySessions(),week=wwIntelWeekSessions(),exam=wwIntelUpcomingExam(),examRisk=wwIntelExamRisk(exam?exam.exam:null),errors=getErrorsDueToday(),weak=wwIntelWeakTopic(),tasks=getTasksForToday();var weeklyMinutes=week.reduce(function(a,x){return a+(Number(x.duration)||0)},0);return{mission:mission,todaySessions:today.length,weekSessions:week.length,weeklyMinutes:weeklyMinutes,exam:exam,examRisk:examRisk,errors:errors,weak:weak,tasks:tasks}}

// V50 Intelligence 2.0 — continuous signal/scoring engine.
// Planning remains the source of truth; this layer only ranks actions.
function wwIntelClamp(n,min,max){return Math.max(min,Math.min(max,n))}
function wwIntelScoreSignals(s){
  var out=[],examScore=0,missionScore=0,errorScore=0,topicScore=0,taskScore=0,momentumScore=0;
  if(s.exam){
    var d=wwExamDays(s.exam.exam), risk=s.examRisk;
    var urgency=d===null?0:(d<=0?45:d<=1?40:d<=3?32:d<=7?22:d<=14?12:4);
    examScore=wwIntelClamp(urgency + Math.min(35,risk*.55),0,100);
    out.push({key:'exam',label:'Examen',score:Math.round(examScore),detail:d===null?'échéance proche':(d<=1?'≤ 1 jour':d+' jours')});
  }
  if(s.mission.total){
    var pct=s.mission.done/s.mission.total;
    missionScore=wwIntelClamp((1-pct)*70 + (s.mission.done===0?8:0) + (s.todaySessions===0?10:0),0,100);
    out.push({key:'mission',label:'Mission',score:Math.round(missionScore),detail:s.mission.done+'/'+s.mission.total});
  }
  if(s.errors.length){
    errorScore=wwIntelClamp(28+s.errors.length*10+(s.todaySessions===0?8:0),0,100);
    out.push({key:'errors',label:'Erreurs',score:Math.round(errorScore),detail:s.errors.length+' dues'});
  }
  if(s.weak&&s.weak.progress.level<4){
    topicScore=wwIntelClamp((4-s.weak.progress.level)*15 + Math.min(20,s.weak.days*1.1),0,100);
    out.push({key:'topic',label:'Point faible',score:Math.round(topicScore),detail:'Niv. '+s.weak.progress.level+'/4'});
  }
  if(s.tasks.length){
    taskScore=wwIntelClamp(18+s.tasks.length*8+(s.todaySessions===0?8:0),0,75);
    out.push({key:'tasks',label:'Tâches',score:Math.round(taskScore),detail:s.tasks.length+' ouvertes'});
  }
  var target=7,weekly=Math.min(1,s.weekSessions/target),minBoost=Math.min(20,s.weeklyMinutes/30);
  momentumScore=wwIntelClamp(12+weekly*35+minBoost,0,70);
  out.push({key:'momentum',label:'Momentum',score:Math.round(momentumScore),detail:s.weekSessions+' sessions / 7j'});
  return out;
}
function wwIntelligenceNextAction(s){
  var signals=wwIntelScoreSignals(s),best=signals[0];
  signals.forEach(function(x){if(x.score>best.score)best=x});
  if(best.key==='exam'&&s.exam)return{type:'exam',title:'Prépare ton prochain examen',desc:s.exam.exam.title+' · '+getExamCountdownLabel(s.exam.dateObj),reason:'Score priorité '+best.score+'/100 · urgence + niveau de préparation.',icon:'🎯',button:'Mode préparation',score:best.score,signals:signals};
  if(best.key==='mission')return{type:'mission',title:'Termine ta mission du jour',desc:s.mission.done+' / '+s.mission.total+' éléments du Planning Hebdo terminés.',reason:'Score priorité '+best.score+'/100 · le Planning Hebdo reste la source de vérité.',icon:'📋',button:'Voir la mission',score:best.score,signals:signals};
  if(best.key==='errors')return{type:'errors',title:'Révise tes erreurs dues',desc:s.errors.length+' erreur'+(s.errors.length>1?'s':'')+' attend'+(s.errors.length>1?'ent':'')+' une révision.',reason:'Score priorité '+best.score+'/100 · signal de fragilité active.',icon:'⚠️',button:'Réviser les erreurs',score:best.score,signals:signals};
  if(best.key==='topic'&&s.weak){var sub=state.subjects.find(function(x){return x.id===s.weak.topic.subject_id});return{type:'topic',title:'Renforce un point faible',desc:s.weak.topic.title+' · Niveau '+s.weak.progress.level+'/4',reason:'Score priorité '+best.score+'/100 · maîtrise basse + ancienneté.',icon:'🧠',button:'Ouvrir la matière',subjectId:s.weak.topic.subject_id,score:best.score,signals:signals}};
  if(best.key==='tasks')return{type:'tasks',title:'Nettoie tes tâches du jour',desc:s.tasks.length+' tâche'+(s.tasks.length>1?'s':'')+' encore ouverte'+(s.tasks.length>1?'s':''),reason:'Score priorité '+best.score+'/100 · tâches planifiées non terminées.',icon:'✅',button:'Voir Planning',score:best.score,signals:signals};
  if(s.weekSessions>0)return{type:'momentum',title:'Consolide ton momentum',desc:s.weekSessions+' session'+(s.weekSessions>1?'s':'')+' cette semaine · '+s.weeklyMinutes+' min',reason:'Aucune urgence forte · ton meilleur levier est la régularité.',icon:'🔥',button:'Voir les Stats',score:best.score,signals:signals};
...
function renderIntelligenceBrief(){
  ...
  else if(a.type==='tasks'||a.type==='mission')action='<button class="ww-intel-action btn-primary" data-route="planning">'+a.button+' →</button>';
  else if(a.type==='momentum')action='<button class="ww-intel-action btn-primary" data-route="stats">'+a.button+' →</button>';
  else action='<button class="ww-intel-action btn-primary" data-route="master">'+a.button+' →</button>';
  var bars=a.signals.filter(function(x){return x.key!=='momentum'||x.score>15}).sort(function(x,y){return y.score-x.score}).slice(0,5).map(function(x){return '<div class="ww-intel-signal"><div><span>'+x.label+'</span><small>'+x.detail+'</small></div><b>'+x.score+'</b><i><em style="width:'+x.score+'%"></em></i></div>'}).join('');
  var examLine=s.exam?'<span>🎯 Examen: '+s.exam.exam.title+' · '+getExamCountdownLabel(s.exam.dateObj)+'</span>':'';
...
function renderPersonalAnalytics(){
  var trend=wwPARecentTrend(),cons=wwPAConsistency(),momentum=wwPAMomentum(),week=wwPAWeekdayProfile(),hours=wwPAHourProfile(),subs=wwPASubjectProfile(),proj=wwPAProjection(),ins=wwPAInsights();
  var activeHour=hours.filter(function(x){return x.sessions}).sort(function(a,b){return b.minutes-a.minutes})[0];
  var weekMax=Math.max.apply(null,week.map(function(x){return x.minutes}).concat([1]));
  var subjectTotal=Math.max(1,subs.reduce(function(a,x){return a+x.minutes},0));
  var insightHTML=ins.map(function(x){return '<div class="ww-pa-insight"><span class="ww-pa-insight-icon">'+x.icon+'</span><div><b>'+x.title+'</b><p>'+x.text+'</p></div></div>'}).join('');
  var weekHTML=week.map(function(x){return '<div class="ww-pa-day"><div class="ww-pa-day-label">'+x.label+'</div><div class="ww-pa-day-track"><i style="height:'+Math.max(x.minutes?8:2,Math.round(x.minutes/weekMax*100))+'%"></i></div><small>'+x.minutes+'m</small></div>'}).join('');
  var subHTML=subs.slice(0,6).map(function(x){return '<div class="ww-pa-sub"><div><b>'+x.name+'</b><small>'+x.sessions+' sessions · '+x.avg+' min/session</small></div><strong>'+x.minutes+'m</strong><div class="ww-pa-sub-bar"><i style="width:'+Math.round(x.minutes/subjectTotal*100)+'%"></i></div></div>'}).join('');
  return '<div class="ww-pa">'+
    '<div class="ww-pa-hero card"><div class="ww-pa-hero-top"><div><div class="card-title">🧬 Personal Analytics</div><div class="ww-pa-subtitle">Ton comportement d’étude sur les données déjà enregistrées</div></div><span class="ww-pa-score">'+momentum.score+'/100<br><small>'+momentum.label+'</small></span></div><div class="ww-pa-kpis"><div><b>'+trend.current+'m</b><span>7 derniers jours</span></div><div><b>'+cons.rate+'%</b><span>Consistency 28j</span></div><div><b>'+proj.weekly+'m</b><span>rythme hebdo</span></div><div><b>'+cons.bestRun+'j</b><span>meilleur run</span></div></div></div>'+
    '<div class="card"><div class="card-title">📈 Trend & projection</div><div class="ww-pa-trend '+(trend.delta>=0?'up':'down')+'"><b>'+ (trend.delta>=0?'↗':'↘') +' '+(trend.delta>=0?'+':'')+trend.delta+'%</b><span>vs semaine précédente</span></div><div class="ww-pa-proj"><div><span>Rythme actuel</span><b>'+proj.daily+' min/jour</b></div><div><span>Objectif 7h</span><b>'+proj.pace+'%</b></div></div></div>'+
    '<div class="card"><div class="card-title">📅 Tes jours les plus efficaces</div><div class="ww-pa-week">'+weekHTML+'</div></div>'+
    '<div class="card"><div class="card-title">⚖️ Équilibre des matières</div>'+ (subHTML||'<div class="chart-empty">Pas assez de sessions avec matière associée.</div>') +'</div>'+
    '<div class="card"><div class="card-title">⏰ Créneaux de travail</div><div class="ww-pa-hours">'+(activeHour?'<div class="ww-pa-focus"><span>Créneau dominant</span><b>'+String(activeHour.hour).padStart(2,'0')+'h</b><small>'+activeHour.minutes+' min enregistrées</small></div>':'<div class="chart-empty">Pas assez de données horaires.</div>')+'</div></div>'+
    '<div class="card"><div class="card-title">🧠 Ce que tes données disent</div><div class="ww-pa-insights">'+insightHTML+'</div></div>'+
  '</div>';
}
...
function attachAppEvents(){
  if(!window._wwFeatureControllers&&window.WWFeatureControllers){
    window._wwFeatureControllers=WWFeatureControllers.create({
      ...
      DEFAULT_SCHEDULE:DEFAULT_SCHEDULE,getProgress:getProgress,computeMasteryScore:computeMasteryScore,getLang:getLang,getCardsDueToday:getCardsDueToday,
  ...
  // V61.8: Android/PWA uses the native <label for=file> activation path.
  // No programmatic input.click() is required, preserving the user gesture.
...
var wwChatbot=window.WWChatbot?WWChatbot.create({messages:chatMsgs,input:chatInput,state:state,format:wwChatFormat,upgrade:wwUpgradeIcons,totalHours:getTotalStudyHours,average:getAveragePerDay,streak:getStudyStreakFromSessions,langDone:langTotalDone,progDone:progTotalDone,programmingCount:PROGRAMMING_TOPICS.length}):null;
