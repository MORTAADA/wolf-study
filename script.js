...
async function wwOpenResourceInApp(sid,rid){
  var r=null;
  Object.keys(wwResourceFolders(sid)).some(function(f){
    r=(Array.isArray(wwResourceFolders(sid)[f])?wwResourceFolders(sid)[f]:[]).find(function(x){return x.id===rid});
    return !!r;
  });
...
// ============================================================
function getErrorsDueToday(){var now=Date.now();return (state.errors||[]).filter(function(e){if(!e||e.status==='mastered')return false;if(!e.next_review)return true;var d=new Date(e.next_review);if(isNaN(d.getTime()))return true;return d.getTime()<=now}).sort(function(a,b){var da=Date.parse(a.next_review)||0,db=Date.parse(b.next_review)||0;return da-db})}
function scheduleNextReview(err){if(!err)return;var base=new Date();var days=0;if(err.status==='mastered'){err.next_review=null;return}if(err.revisions<=0){days=0}else{var intervals=[1,3,7,14,30];days=intervals[Math.min(err.revisions-1,intervals.length-1)]}base.setDate(base.getDate()+days);err.next_review=wwLocalDateISO(base)}
function addError(desc,subjectId,topicId,cause,correction,difficulty){var err={id:'err_'+generateId(),description:desc,subject_id:subjectId||null,topic_id:topicId||null,cause:cause||'other',correction:correction||'',difficulty:difficulty||'medium',status:'to_review',revisions:0,max_revisions:3,created_at:wwLocalDateISO(new Date()),last_reviewed:null,next_review:wwLocalDateISO(new Date())};state.errors.push(err);if(window.WWAcademicWriteBridge){window.WWAcademicWriteBridge.error(err).catch(function(e){console.warn('Academic error command failed',e)})}else if(window.WWAcademicEvents)WWAcademicEvents.emit('ERROR_CREATED',{errorId:err.id,subjectId:err.subject_id,topicId:err.topic_id});if(window.WWMastery)window.WWMastery.recordError(state,err.topic_id||null);scheduleNextReview(err);saveState();return err}
function reviewError(id,success){var err=(state.errors||[]).find(function(e){return e.id===id});if(!err)return;err.last_reviewed=wwLocalDateISO(new Date());if(window.WWMastery)window.WWMastery.recordReview(state,err.topic_id||null,success);if(window.WWAcademicEvents)WWAcademicEvents.emit('ERROR_REVIEWED',{errorId:err.id,subjectId:err.subject_id,topicId:err.topic_id,success:!!success});if(success){err.revisions++;if(err.revisions>=err.max_revisions)err.status='mastered';else err.status='in_progress'}else{err.revisions=0;err.status='to_review'}scheduleNextReview(err);saveState()}
...
function wwSafeResourceURL(url){var raw=String(url||'').trim();if(!raw)return '#';try{var u=new URL(raw,location.href);if(u.protocol==='http:'||u.protocol==='https:')return u.href;return '#'}catch(e){return '#'}}
// V79.1 RESOURCE DATA GUARD — old/new schemas may coexist in local storage.
// Every resource subject MUST expose folders whose values are arrays.
function wwNormalizeResourceState(){
  var src=state&&state.resources;
  if(!src||typeof src!=='object'){state.resources={};return state.resources}
  if(Array.isArray(src)){
    var grouped={};
    src.forEach(function(r){if(!r||typeof r!=='object')return;var sid=r.subjectId||r.subject_id||'global',fn=r.folder||'Général';(grouped[sid]||(grouped[sid]={}));(grouped[sid][fn]||(grouped[sid][fn]=[])).push(r)});
    state.resources=grouped; src=grouped;
  }
  Object.keys(src).forEach(function(sid){
    var value=src[sid];
    if(Array.isArray(value)){src[sid]={'Général':value};return}
    if(!value||typeof value!=='object'){src[sid]={};return}
    // Repair a legacy direct-resource object: {id,title,url,...}.
    if(value.id||value.title||value.url||value.fileKey){src[sid]={'Général':[value]};return}
    Object.keys(value).forEach(function(fn){
      var bucket=value[fn];
      if(Array.isArray(bucket))return;
      if(bucket&&typeof bucket==='object'&&(bucket.id||bucket.title||bucket.url||bucket.fileKey))value[fn]=[bucket];
      else value[fn]=[];
    });
  });
  return state.resources;
}
function wwResourceFolders(sid){
  wwNormalizeResourceState();
  var folders=state.resources[sid];
  if(!folders||typeof folders!=='object'||Array.isArray(folders)){state.resources[sid]={};folders=state.resources[sid]}
  return folders;
}
function getAllResources(){var all=[];wwNormalizeResourceState();Object.keys(state.resources||{}).forEach(function(sid){var s=(state.subjects||[]).find(function(x){return x.id===sid});var folders=wwResourceFolders(sid);Object.keys(folders).forEach(function(fn){var bucket=Array.isArray(folders[fn])?folders[fn]:[];bucket.forEach(function(r){if(!r||typeof r!=='object')return;all.push(Object.assign({},r,{subjectName:s?s.name:'Matière',subjectId:sid,folder:fn,type:r.fileKey?(r.fileType||'pdf'):detectResourceType(r.url)}))})})});return all}
...
function render(){wwPerfMark('ww-render-start');if(window.WWEventBus)WWEventBus.emit('render:start',{route:state.route});try{var root=document.getElementById('root');if(!root)return;if(!state.onboardingDone){var onboardingHTML=renderOnboarding();if(window.WWRenderer)WWRenderer.mount(root,onboardingHTML);else root.innerHTML=onboardingHTML;wwUpgradeIcons(root);attachOnboarding();if(window.WWEventBus)WWEventBus.emit('render:complete',{route:state.route,onboarding:true});return}var html=renderApp();var result=window.WWRenderer?WWRenderer.mount(root,html):null;if(!result)root.innerHTML=html;wwUpgradeIcons(root);attachAppEvents();wwRenderInstallHint();wwPerfMark('ww-render-end');if(window.WWEventBus)WWEventBus.emit('render:complete',{route:state.route,duration:result?result.duration:null})}catch(e){console.error('Render error:',e);var root=document.getElementById('root');if(root){var safeMsg=wwEscapeHTML?wwEscapeHTML(String(e&&e.message||'Erreur inconnue')):String(e&&e.message||'Erreur inconnue').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]});var errHTML='<div style="padding:20px;color:#e86a6a;"><h2>⚠️ Erreur</h2><pre style="font-size:12px;white-space:pre-wrap;">'+safeMsg+'</pre><button id="ww-reload-error" type="button" style="margin-top:20px;background:#2a3f60;color:#fff;border:none;padding:10px 20px;border-radius:10px;">Recharger</button></div>';if(window.WWRenderer)WWRenderer.mount(root,errHTML);else root.innerHTML=errHTML}var rb=document.getElementById('ww-reload-error');if(rb)rb.onclick=function(){location.reload()};if(window.WWEventBus)WWEventBus.emit('render:error',{error:e,route:state.route})}}

function renderOnboarding(){
  var step=state.onboardingStep||0;
  var content='';
  if(step===0){content='<div class="onboarding-step-content"><div class="onboarding-icon onboarding-logo-v2"><img src="logo.webp" alt="White Wolf Scholar" decoding="async" fetchpriority="high"></div><h1 class="onboarding-title">White Wolf Scholar</h1><p class="onboarding-subtitle">Study → Track → Analyze → Master</p><div class="onboarding-features"><div class="onboarding-feature"><span class="of-icon">📚</span><span class="of-text"><strong>Master APCE</strong> — 9 matières</span></div><div class="onboarding-feature"><span class="of-icon">🌱</span><span class="of-text"><strong>3 langues</strong> — A1 → C1</span></div><div class="onboarding-feature"><span class="of-icon">📊</span><span class="of-text"><strong>Stats avancées</strong> — Heatmap + Charts</span></div></div><button class="onboarding-btn" data-onboard-next>Suivant →</button></div>'}
  ...
  else if(step===2){content='<div class="onboarding-step-content"><div class="onboarding-icon">🐺</div><h1 class="onboarding-title">Prêt à commencer</h1><p class="onboarding-subtitle">Ton espace White Wolf Scholar est prêt.</p><div class="onboarding-features"><div class="onboarding-feature"><span class="of-icon">📅</span><span class="of-text">Planning et missions quotidiennes</span></div><div class="onboarding-feature"><span class="of-icon">🧠</span><span class="of-text">Analyse et recommandations</span></div><div class="onboarding-feature"><span class="of-icon">📈</span><span class="of-text">Suivi de ta progression</span></div></div><button class="onboarding-btn" data-onboard-finish>🐺 Commencer →</button><button class="onboarding-skip" data-onboard-skip>Passer</button></div>'}
  var dots='';
...
function attachOnboarding(){var next=document.querySelector('[data-onboard-next]');if(next)next.onclick=function(){state.onboardingStep=(state.onboardingStep||0)+1;render()};var prev=document.querySelector('[data-onboard-prev]');if(prev)prev.onclick=function(){state.onboardingStep=Math.max(0,(state.onboardingStep||0)-1);render()};var skip=document.querySelector('[data-onboard-skip]');if(skip)skip.onclick=function(){finishOnboarding()};var finish=document.querySelector('[data-onboard-finish]');if(finish)finish.onclick=function(){finishOnboarding()};document.querySelectorAll('[data-onboard-goal]').forEach(function(el){el.onclick=function(){state.onboardingData.goal=this.dataset.onboardGoal;render()}})}
function finishOnboarding(){state.onboardingDone=true;state.onboardingStep=0;saveState();render()}

function renderApp(){
  var a=state.route;
  var inG=(a==='growth'||a==='quran'||a==='language'||a==='lesson'||a==='programming'||a==='flashcards');
  return '<div class="app">'+
    '<div class="header"><h1 class="app-brand"><img class="app-logo" src="logo.webp" alt="White Wolf Scholar"> <span class="app-brand-text">White Wolf</span></h1>'+
      '<div style="position:relative;display:flex;align-items:center;gap:8px;">'+
        '<span class="settings-btn ww-global-search-trigger" data-global-search title="Recherche globale">⌕</span>'+        '<span class="settings-btn" id="settings-trigger">⚙️</span>'+
        '<div class="settings-dropdown" id="settings-menu">'+
          '<div class="section-label">Préférences</div>'+
          '<div class="item" data-notification-settings><span class="icon">🔔</span> Notifications <span class="toggle-status">'+(window.WWNotifications&&window.WWNotifications.get().enabled?'✅':'❌')+'</span></div>'+          '<div class="item" data-setting="showSmartRevision"><span class="icon">🧠</span> Recommandations <span class="toggle-status">'+(state.settings.showSmartRevision?'✅':'❌')+'</span></div>'+          '<div class="divider"></div>'+
          '<div class="section-label">Actions</div>'+          '<div class="item" data-modal="about"><span class="icon">ℹ️</span> À propos</div>'+
        '</div></div></div>'+
    renderContent(a)+
...
function renderDashboardExams(){
  ...
  var rows=upcoming.map(function(x,i){
    ...
    var room=e.room?'<span class="ww-exam-meta-item">⌂ Salle '+wwEscapeHTML(e.room)+'</span>':'';
    return '<div class="ww-dashboard-exam'+cls+'" data-route="planning">'+
      '<div class="ww-exam-icon">📝</div>'+
      '<div class="ww-exam-main"><div class="ww-exam-title">'+wwEscapeHTML(e.title)+'</div>'+ 
      '<div class="ww-exam-meta"><span class="ww-exam-meta-item">📅 '+dateLabel+timeLabel+'</span>'+ (sub?'<span class="ww-exam-meta-item">📚 '+wwEscapeHTML(sub.name)+'</span>':'')+room+'</div></div>'+ 
...
function wwTaskDeadlineDateTime(t){return window.WWTaskEngine?WWTaskEngine.deadlineDateTime(t):null}
function wwTaskStatus(t){return window.WWTaskEngine?WWTaskEngine.taskStatus(t,wwLocalDateISO(new Date()),new Date()):'À faire'}
...
function getErrorsByStatus(status){var all=Array.isArray(state.errors)?state.errors:[];if(!status||status==='all')return all;return all.filter(function(e){return e&&e.status===status})}
function computeErrorStats(){var x=window.WWAnalytics&&typeof window.WWAnalytics.errorStats==='function'?window.WWAnalytics.errorStats(state):{total:(state.errors||[]).length,due:0,byStatus:{to_review:0,in_progress:0,mastered:0}};x.byStatus=x.byStatus||{};x.to_review=Number(x.byStatus.to_review||0);x.in_progress=Number(x.byStatus.in_progress||0);x.mastered=Number(x.byStatus.mastered||0);return x}
...
function renderReviewSession(){
  ...
  return '<div class="review-session"><div class="rs-progress"><span>Révision '+(session.currentIdx+1)+'/'+session.errors.length+'</span><span>Révisions: '+err.revisions+'/'+err.max_revisions+'</span></div><div class="rs-question">'+wwEscapeHTML(err.description)+'</div><div style="font-size:12px;color:#8ba2c0;text-align:center;margin-bottom:12px;">'+meta+'</div>'+(err.correction?'<div class="rs-correction">✅ '+wwEscapeHTML(err.correction)+'</div>':'')+'<div style="font-size:13px;color:#c8d6e5;text-align:center;margin-bottom:14px;">Maîtrises-tu ?</div><div class="rs-actions"><button class="rs-btn-no" data-review-result="no">❌ Non</button><button class="rs-btn-yes" data-review-result="yes">✅ Oui</button></div><button class="btn-outline btn-small" data-end-review style="width:100%;justify-content:center;margin-top:10px;">Arrêter</button></div>';
}

function wwScheduleToMinutes(t){return window.WWTaskEngine?WWTaskEngine.scheduleToMinutes(t):0}
function wwScheduleDuration(c){return window.WWTaskEngine?WWTaskEngine.scheduleDuration(c):0}
...
function renderResources(){
  ...
  var search='<div class="res-search"><input type="text" id="res-search-input" placeholder="🔍 Rechercher..." value="'+wwEscapeHTML(state.resSearch||'')+'"><span class="search-icon">🔍</span></div>';
  ...
  else{var grouped={};filtered.forEach(function(r){if(!grouped[r.subjectName])grouped[r.subjectName]=[];grouped[r.subjectName].push(r)});content=Object.keys(grouped).map(function(subjectName){var items=grouped[subjectName],safeGroup=wwEscapeHTML(subjectName);var isOpen=state.resOpenGroups[subjectName]!==false;var itemsHTML=isOpen?items.map(function(r){var type=r.type;var icon=getResourceIcon(type);var domain=getDomainName(r.url);var safeURL=wwSafeResourceURL(r.url);var star=r.favorite?'⭐':'☆';var studied=r.studied?'active':'';var studyTime=Number(r.studyMinutes||0);var openLabel=r.fileKey?'📄 Ouvrir fichier':'🔗 Ouvrir';var topic=(state.topics||[]).find(function(t){return t.id===r.topic_id})||{};return '<div class="res-card type-'+wwEscapeHTML(type)+' '+(r.studied?'is-studied':'')+'"><div class="rc-star '+(r.favorite?'active':'')+'" data-toggle-fav="'+wwEscapeHTML(r.subjectId)+'|'+wwEscapeHTML(r.id)+'">'+star+'</div><div class="rc-header"><div class="rc-icon">'+icon+'</div><div class="rc-info"><div class="rc-title">'+wwEscapeHTML(r.title)+'</div><div class="rc-meta"><span class="rc-badge">'+wwEscapeHTML(getResourceTypeLabel(type))+'</span>'+(r.fileName?'<span>📎 '+wwEscapeHTML(r.fileName)+'</span>':'')+(r.topic_id?'<span>🎯 '+wwEscapeHTML(topic.title||'')+'</span>':'')+(domain?'<span>🌐 '+wwEscapeHTML(domain)+'</span>':'')+'</div></div></div><div class="rc-study"><button class="btn-study '+studied+'" data-toggle-studied="'+wwEscapeHTML(r.subjectId)+'|'+wwEscapeHTML(r.id)+'">'+(r.studied?'✓ Étudiée':'○ Marquer étudiée')+'</button><span class="study-time">⏱ '+studyTime+' min</span><button class="btn-time" data-log-resource-time="'+wwEscapeHTML(r.subjectId)+'|'+wwEscapeHTML(r.id)+'">+ Temps</button></div><div class="rc-actions">'+(r.fileKey?'<button class="btn-open" data-open-resource="'+wwEscapeHTML(r.subjectId)+'|'+wwEscapeHTML(r.id)+'">'+openLabel+'</button>':'<a href="'+wwEscapeHTML(safeURL)+'" target="_blank" rel="noopener noreferrer" class="btn-open">'+openLabel+'</a>')+(r.url?'<button class="btn-copy" data-copy-url="'+wwEscapeHTML(r.url)+'">📋</button>':'')+(window.WWDocumentIntel&&window.WWDocumentIntel.get&&window.WWDocumentIntel.get(r.id)?'<button class="btn-open" data-map-resource="'+wwEscapeHTML(r.id)+'">🧭 أقسام</button><button class="btn-open" data-search-resource="'+wwEscapeHTML(r.id)+'">🔎 بحث</button>':'<button class="btn-open" data-index-resource="'+wwEscapeHTML(r.id)+'">🧠 فهرسة</button>')+'<button class="btn-delete" data-delete-resource="'+wwEscapeHTML(r.subjectId)+'|'+wwEscapeHTML(r.id)+'">🗑️</button></div></div>'}).join(''):'';return '<div style="margin-bottom:16px;"><div class="res-group-header '+(isOpen?'open':'')+'" data-toggle-group="'+safeGroup+'"><div class="rgh-title">📚 '+safeGroup+'</div><div style="display:flex;align-items:center;gap:10px;"><span class="rgh-count">'+items.length+'</span><span class="rgh-arrow">'+(isOpen?'▼':'▶')+'</span></div></div>'+itemsHTML+'</div>'}).join('')}
...
function attachAppEvents(){
  ...
  document.querySelectorAll('[data-open-topic]').forEach(function(el){el.onclick=function(e){if(e.target.closest('[data-scope-topic],[data-topic-up],[data-topic-down]'))return;e.preventDefault();navigate('topic',{topicId:this.dataset.openTopic})};el.onkeydown=function(e){if((e.key==='Enter'||e.key===' ')&&!e.target.closest('[data-scope-topic],[data-topic-up],[data-topic-down]')){e.preventDefault();navigate('topic',{topicId:this.dataset.openTopic})}}});
  ...
  document.querySelectorAll('[data-setting]').forEach(function(el){el.onclick=function(){var s=this.dataset.setting;if(s==='showSmartRevision'){state.settings.showSmartRevision=!state.settings.showSmartRevision}state.isSettingsOpen=false;document.getElementById('settings-menu').classList.remove('open');saveState();render()}});
  document.querySelectorAll('[data-notification-settings]').forEach(function(el){el.onclick=async function(){if(!window.WWNotifications){showToast('🔔 Notifications indisponibles');return}var p=await window.WWNotifications.enable();if(p==='granted'){showToast('🔔 Notifications activées');window.WWNotifications.start(state)}else{showToast('🔕 Autorisation des notifications refusée')}state.isSettingsOpen=false;document.getElementById('settings-menu').classList.remove('open');render()}});
  ...
  document.querySelectorAll('[data-topic-up]').forEach(function(el){el.onclick=function(e){e.stopPropagation();var tid=this.dataset.topicUp;var p=getProgress(tid);if(p.level<4){p.level++;p.score=wwMasteryScore(tid);p.last_studied=new Date().toISOString();state.progress[tid]=p;if(window.WWMastery)window.WWMastery.syncTopic(state,tid);if(window.WWAcademicEvents)WWAcademicEvents.emit('MASTERY_CHANGED',{topicId:tid,mastery:p.score,stage:p.level,source:'topic-up'});state.xp+=5;saveState();render()}}});
  document.querySelectorAll('[data-topic-down]').forEach(function(el){el.onclick=function(e){e.stopPropagation();var tid=this.dataset.topicDown;var p=getProgress(tid);if(p.level>0){p.level--;p.score=wwMasteryScore(tid);state.progress[tid]=p;if(window.WWMastery)window.WWMastery.syncTopic(state,tid);if(window.WWAcademicEvents)WWAcademicEvents.emit('MASTERY_CHANGED',{topicId:tid,mastery:p.score,stage:p.level,source:'topic-down'});saveState();render()}}});
  ...
  document.querySelectorAll('[data-save-task]').forEach(function(el){el.onclick=async function(){
    ...
    var subjectId=(document.getElementById('task-subject')||{}).value||'';
    var topicId=(document.getElementById('task-topic')||{}).value||'';
    var linkedTopic=topicId?(state.topics||[]).find(function(t){return t.id===topicId}):null;
    if(linkedTopic){if(subjectId&&linkedTopic.subject_id!==subjectId){showToast('⚠️ Le chapitre ne correspond pas à la matière');return}subjectId=linkedTopic.subject_id||subjectId}
    ...
    var task={id:generateId(),text:text,title:text,date:date,time:time,deadlineTime:deadlineTime,estimatedMinutes:estimatedMinutes,priority:priority,subjectId:subjectId||null,subject_id:subjectId||null,topicId:topicId||null,topic_id:topicId||null,isDone:false,status:'todo',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
    state.tasks=Array.isArray(state.tasks)?state.tasks:[];state.tasks.push(task);state.modal=null;
    if(window.WWAcademicEvents)WWAcademicEvents.emit('TASK_CREATED',{taskId:task.id,date:date,time:time,deadlineTime:deadlineTime,estimatedMinutes:estimatedMinutes,priority:priority});
    await saveState();showToast('✅ Tâche ajoutée');render();
  }});
  document.querySelectorAll('[data-task-done]').forEach(function(el){el.onclick=async function(e){e.stopPropagation();var id=this.dataset.taskDone,t=(state.tasks||[]).find(function(x){return x.id===id});if(!t)return;t.isDone=true;t.status='done';t.completedAt=new Date().toISOString();t.updatedAt=new Date().toISOString();state.xp+=5;if(window.WWAcademicEvents)WWAcademicEvents.emit('TASK_COMPLETED',{taskId:t.id,subjectId:t.subject_id||t.subjectId||null,topicId:t.topic_id||t.topicId||null});await saveState();render();}});
  document.querySelectorAll('[data-task-delete]').forEach(function(el){el.onclick=async function(e){e.stopPropagation();var id=this.dataset.taskDelete;if(!confirm('Supprimer cette tâche ?'))return;state.tasks=(state.tasks||[]).filter(function(x){return x.id!==id});await saveState();render();}});
  document.querySelectorAll('[data-task-start]').forEach(function(el){el.onclick=async function(){var t=state.tasks.find(function(x){return x.id===this.dataset.taskStart},this);if(t){t.status='in_progress';t.isDone=false;t.startedAt=t.startedAt||new Date().toISOString();t.updatedAt=new Date().toISOString();await saveState();render()}}});
  document.querySelectorAll('[data-task-stop]').forEach(function(el){el.onclick=async function(){var t=state.tasks.find(function(x){return x.id===this.dataset.taskStop},this);if(t){t.status='todo';t.updatedAt=new Date().toISOString();await saveState();render()}}});
  document.querySelectorAll('[data-task-undone]').forEach(function(el){el.onclick=async function(){var t=state.tasks.find(function(x){return x.id===this.dataset.taskUndone},this);if(t){t.isDone=false;t.status='todo';t.updatedAt=new Date().toISOString();await saveState();render()}}});
  ...
  document.querySelectorAll('[data-save-resource]').forEach(function(el){el.onclick=async function(){var sid=document.getElementById('resource-subject').value;var topicId=(document.getElementById('resource-topic')||{}).value||'';var title=document.getElementById('resource-title').value.trim();var url=document.getElementById('resource-url').value.trim();var fileInput=document.getElementById('resource-file');var file=(state.pendingResourceFile)||(fileInput&&fileInput.files?fileInput.files[0]:null);var handle=state.pendingResourceHandle;if(!sid||!title||(!url&&!file&&!handle)){alert('Ajoute un lien ou un fichier');return}var folders=wwResourceFolders(sid);if(!Array.isArray(folders['Général']))folders['Général']=[];var r={id:generateId(),title:title,url:url||'',tag:'📚',dateAdded:wwLocalDateISO(new Date()),favorite:false,studied:false,studyMinutes:0,topic_id:topicId||null};if(file||handle){r.fileKey='rf_'+r.id;r.fileName=(file&&file.name)||'Fichier sélectionné';r.fileType=((file&&file.type)||'').indexOf('pdf')!==-1?'pdf':((file&&file.type)||'').indexOf('word')!==-1?'doc':((file&&file.type)||'').indexOf('image')!==-1?'image':((file&&file.type)||'').indexOf('audio')!==-1?'audio':((file&&file.type)||'').indexOf('video')!==-1?'video':'doc';try{await fileSet(r.fileKey,handle||file)}catch(e){alert('Impossible d’enregistrer le fichier dans le stockage local');return}}folders['Général'].push(r);if(window.WWAcademicWriteBridge)window.WWAcademicWriteBridge.commit('resources',Object.assign({},r,{subjectId:sid,topicId:topicId||null,folder:'Général'})).catch(function(e){console.warn('Academic resource command failed',e)});state.modal=null;state.pendingResourceHandle=null;state.pendingResourceFile=null;showToast('✅ Ressource ajoutée');await saveState();render()}});
  ...
  document.querySelectorAll('[data-toggle-studied]').forEach(function(el){el.onclick=function(){var parts=this.dataset.toggleStudied.split('|');var r=null;Object.keys(wwResourceFolders(parts[0])).some(function(f){r=(Array.isArray(wwResourceFolders(parts[0])[f])?wwResourceFolders(parts[0])[f]:[]).find(function(x){return x.id===parts[1]});return !!r});if(r){r.studied=!r.studied;saveState();render()}}});
  ...
  document.querySelectorAll('[data-save-resource-time]').forEach(function(el){el.onclick=function(){var mins=Math.max(1,Number(document.getElementById('resource-time-min').value)||0);var sid=this.dataset.sid,rid=this.dataset.rid,r=null;Object.keys(wwResourceFolders(sid)).some(function(f){r=(Array.isArray(wwResourceFolders(sid)[f])?wwResourceFolders(sid)[f]:[]).find(function(x){return x.id===rid});return !!r});if(r){r.studyMinutes=Number(r.studyMinutes||0)+mins;r.studied=true;state.modal=null;showToast('⏱️ '+mins+' min enregistrées');saveState();render()}}});
  ...
  document.querySelectorAll('[data-delete-resource]').forEach(function(el){el.onclick=function(e){e.stopPropagation();if(!confirm('Supprimer ?'))return;var parts=this.dataset.deleteResource.split('|');var sid=parts[0],rid=parts[1];if(state.resources[sid]){Object.keys(wwResourceFolders(sid)).forEach(function(f){var bucket=Array.isArray(wwResourceFolders(sid)[f])?wwResourceFolders(sid)[f]:[];wwResourceFolders(sid)[f]=bucket.filter(function(r){if(r.id===rid&&r.fileKey){fileDelete(r.fileKey);if(window.WWDocumentIntel&&window.WWDocumentIntel.clear)window.WWDocumentIntel.clear(r.id)}return r.id!==rid})})}saveState();render()}});
}
...
function renderBootStatus(message,detail,showRecovery){
  ...
  if(!root)return;
  var actions=showRecovery===false?'':'<div class="ww-boot-actions"><button type="button" id="ww-boot-retry">↻ Réessayer</button><button type="button" id="ww-boot-reset">Réinitialiser le cache local</button></div>';
  root.innerHTML='<div class="ww-boot-screen"><div class="ww-boot-mark">🐺</div><h1>WHITE WOLF</h1><p>'+wwEscapeHTML(String(message||''))+'</p>'+(detail?'<small>'+wwEscapeHTML(String(detail))+'</small>':'')+actions+'</div>';
  var retry=document.getElementById('ww-boot-retry');if(retry)retry.onclick=function(){location.reload()};
  var reset=document.getElementById('ww-boot-reset');if(reset)reset.onclick=function(){try{localStorage.removeItem('wwAppStateFallback')}catch(e){};location.reload()};
  wwUpgradeIcons(root);
}
