...
//  NOTIFICATIONS
// ============================================================
function requestNotificationPermission(){if(window.WWNotifications&&WWNotifications.requestPermission)return WWNotifications.requestPermission();if(!('Notification' in window))return Promise.resolve('unsupported');return Notification.requestPermission()}
function sendNotification(title,body,options){if(window.WWNotifications&&WWNotifications.show)return WWNotifications.show({id:'manual_'+Date.now(),category:'intelligence',type:'info',title:title,text:body,icon:'🔔'});if(!state.settings||!state.settings.notifications||!('Notification' in window)||Notification.permission!=='granted')return;try{return new Notification(title,Object.assign({body:body,icon:'./icons/icon-192.png'},options||{}))}catch(e){}}
function computeNotifications(){if(window.WWNotifications&&WWNotifications.build)return WWNotifications.build(state,new Date());return []}
function updateNotifications(){try{var notifs=window.WWNotifications?WWNotifications.tick(state,true):computeNotifications();state.notifications=notifs;if(state.settings&&state.settings.notifications&&typeof saveState==='function')saveState()}catch(e){console.warn('Notification update failed',e)}}
    ...
    '<div class="header"><h1 class="app-brand"><img class="app-logo" src="logo.svg" alt="White Wolf Scholar"> <span class="app-brand-text">White Wolf</span></h1>'+
      '<div style="position:relative;display:flex;align-items:center;gap:8px;">'+
        ...
        '<div class="settings-dropdown" id="settings-menu">'+
          ...
          '<div class="item" data-modal="notifications"><span class="icon">📬</span> Centre de notifications</div><div class="item" data-modal="notification-settings"><span class="icon">⚙️</span> Réglages des notifications</div>'+
...
function renderDashboard(){
  ...
  return '<div class="dashboard-shell">'+wwHero+
    ...
    '<div class="card"><div class="card-title">📋 Tâches du jour <span class="badge">'+new Date().toLocaleDateString('fr-FR')+'</span></div>'+
      (tasks.length?tasks.map(function(t){return '<div class="task-item"><div class="task-left"><div class="task-text">'+t.text+'</div><div class="task-meta">'+(t.deadlineTime?'⏰ limite '+t.deadlineTime:(t.time?'▶️ '+t.time:''))+' • '+(t.estimatedMinutes?('⏱️ '+t.estimatedMinutes+' min • '):'')+t.priority+'</div></div><div class="task-right"><span class="task-priority '+t.priority+'">'+t.priority+'</span><button class="btn-small btn-outline" data-task-done="'+t.id+'">✅</button><button class="btn-small btn-outline" data-task-delete="'+t.id+'">🗑️</button></div></div>'}).join(''):'<div class="text-muted text-small">Aucune tâche.</div>')+
    '</div>'+
...
function renderModal(){
  ...
  if(m.type==='notification-settings'){var ns=window.WWNotifications?WWNotifications.settings():{enabled:true,categories:{},push:{}};var labels={planning:'🧭 Planning / Sessions',tasks:'🎯 Tâches / Deadlines',exams:'📚 Examens',review:'🧠 Smart Review / Mastery',resources:'📖 Ressources',intelligence:'🐺 White Wolf Intelligence',progress:'📈 Progression / Streak',weekly:'📊 Rapport hebdomadaire'};var rows=Object.keys(labels).map(function(k){return '<label style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.06);gap:12px"><span>'+labels[k]+'</span><input type="checkbox" data-notif-cat="'+k+'" '+(ns.categories&&ns.categories[k]?'checked':'')+'></label>'}).join('');var ps=window.WWNotifications?WWNotifications.pushStatus():{};return '<div class="modal-overlay"><div class="modal-content"><span class="close-btn" data-close-modal>❌</span><h3>🔔 Réglages des notifications</h3><p class="text-muted text-small">Choisis exactement ce que White Wolf peut te notifier. Les notifications ne modifient jamais automatiquement ton Planning.</p><div style="display:grid;gap:2px">'+rows+'</div><div style="margin-top:14px"><label style="display:flex;justify-content:space-between;align-items:center"><span>🌙 Mode silencieux</span><input type="checkbox" id="notif-quiet" '+(ns.quietHours&&ns.quietHours.enabled?'checked':'')+'></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px"><input id="notif-quiet-start" type="time" value="'+((ns.quietHours&&ns.quietHours.start)||'22:00')+'"><input id="notif-quiet-end" type="time" value="'+((ns.quietHours&&ns.quietHours.end)||'07:00')+'"></div></div><div style="margin-top:14px;padding-top:12px;border-top:1px solid rgba(255,255,255,.08)"><strong>📡 Web Push — appareil fermé</strong><div class="text-muted text-small" style="margin:7px 0">'+(ps.configured?'Gateway configuré · permission: '+ps.permission:'Gateway Cloudflare prêt à être activé')+'</div><button class="btn-primary" type="button" data-enable-web-push>🔔 Activer les notifications Push</button><div class="text-muted text-small" style="margin-top:7px">Le Gateway est préconfiguré. Aucun VAPID privé ni ADMIN_TOKEN n’est stocké dans l’application.</div></div><div class="modal-actions"><button class="btn-outline" data-close-modal>Fermer</button><button class="btn-primary" data-save-notif-settings>💾 Enregistrer</button></div></div></div>'}
...
function attachAppEvents(){
  ...
  document.querySelectorAll('[data-setting]').forEach(function(el){el.onclick=function(){var s=this.dataset.setting;if(s==='showSmartRevision'){state.settings.showSmartRevision=!state.settings.showSmartRevision}else if(s==='notifications'){state.settings.notifications=!state.settings.notifications;if(window.WWNotifications)WWNotifications.updateSettings({enabled:state.settings.notifications});if(state.settings.notifications){requestNotificationPermission().then(function(r){if(r==='granted')showToast('🔔 Activées')})}}state.isSettingsOpen=false;document.getElementById('settings-menu').classList.remove('open');saveState();render()}});
  ...
  document.querySelectorAll('[data-enable-web-push]').forEach(function(el){el.onclick=async function(){if(!window.WWNotifications){showToast('❌ Système de notifications indisponible');return}el.disabled=true;el.textContent='⏳ Activation...';try{var gateway='https://white-wolf-scholar-push-gateway.white-wolf-scholar.workers.dev';var cfg=await WWNotifications.configureFromGateway(gateway);if(!cfg.ok)throw new Error(cfg.reason||'gateway_config_failed');var r=await WWNotifications.subscribePush();if(!r.ok)throw new Error(r.reason||('HTTP '+r.status));showToast('🔔 Web Push activé sur cet appareil');render()}catch(err){console.error('Web Push activation failed',err);el.disabled=false;el.textContent='🔔 Activer les notifications Push';showToast('❌ Push: '+(err.message||err))}}});
  document.querySelectorAll('[data-save-notif-settings]').forEach(function(el){el.onclick=function(){if(window.WWNotifications){var cats={};document.querySelectorAll('[data-notif-cat]').forEach(function(x){cats[x.dataset.notifCat]=x.checked});WWNotifications.updateSettings({categories:cats,quietHours:{enabled:!!document.getElementById('notif-quiet')?.checked,start:(document.getElementById('notif-quiet-start')||{}).value||'22:00',end:(document.getElementById('notif-quiet-end')||{}).value||'07:00'} });}state.modal=null;showToast('🔔 Réglages enregistrés');render()}});
