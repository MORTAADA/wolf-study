...
state.studyScope=state.studyScope||{subjects:{},topics:{}};
...
function wwSubjectActive(subjectId){wwEnsureStudyScope();return state.studyScope.subjects[subjectId]===true}
function wwTopicActive(topic){if(!topic)return false;wwEnsureStudyScope();if(Object.prototype.hasOwnProperty.call(state.studyScope.topics,topic.id))return state.studyScope.topics[topic.id]===true;return wwSubjectActive(topic.subject_id)}
function wwActiveTopics(){return state.topics.filter(function(t){return wwTopicActive(t)})}
function wwActiveSubjectCount(){wwEnsureStudyScope();return state.subjects.filter(function(s){return wwSubjectActive(s.id)}).length}
function wwActiveTopicCount(){return wwActiveTopics().length}
function wwSetSubjectScope(subjectId,active){wwEnsureStudyScope();state.studyScope.subjects[subjectId]=!!active;if(!active){state.topics.filter(function(t){return t.subject_id===subjectId}).forEach(function(t){state.studyScope.topics[t.id]=false})}else{state.topics.filter(function(t){return t.subject_id===subjectId}).forEach(function(t){if(!Object.prototype.hasOwnProperty.call(state.studyScope.topics,t.id))state.studyScope.topics[t.id]=true})}saveState();render()}
function wwSetTopicScope(topicId,active){wwEnsureStudyScope();var t=state.topics.find(function(x){return x.id===topicId});if(!t)return;state.studyScope.topics[topicId]=!!active;if(active)state.studyScope.subjects[t.subject_id]=true;saveState();render()}
function wwActivateAllSubjectTopics(subjectId,active){wwEnsureStudyScope();state.studyScope.subjects[subjectId]=!!active;state.topics.filter(function(t){return t.subject_id===subjectId}).forEach(function(t){state.studyScope.topics[t.id]=!!active});saveState();render()}
...
function renderAdaptiveRevisionCard(){
  ...
  if(!items.length)return '<div class="card ww-adaptive-card"><div class="ww-adaptive-head"><div><div class="card-title">🧠 Révision adaptative</div><div class="ww-adaptive-sub">Aucune priorité forte détectée pour le moment.</div></div><span class="ww-adaptive-badge">63.6</span></div><button class="btn-outline" data-stats-tab="adaptive" style="width:100%;justify-content:center;margin-top:10px;">Voir le moteur</button></div>';
...
window.WWAppCore={state:state,render:render,navigate:navigate,version:'63.6',events:window.WWEventBus,renderer:window.WWRenderer};
