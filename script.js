...
function renderModal(){
  ...
  if(m.type==='notification-settings'){var ns=window.WWNotifications?WWNotifications.settings():{enabled:true,categories:{}};var labels={planning:'🧭 Planning / Sessions',tasks:'🎯 Tâches / Deadlines',exams:'📚 Examens',review:'🧠 Smart Review / Mastery',resources:'📖 Ressources',intelligence:'🐺 White Wolf Intelligence',progress:'📈 Progression / Streak',weekly:'📊 Rapport hebdomadaire'};var rows=Object.keys(labels).map(function(k){return '<label style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.06);gap:12px"><span>'+labels[k]+'</span><input type="checkbox" data-notif-cat="'+k+'" '+(ns.categories&&ns.categories[k]?'checked':'')+'></label>'}).join('');return '<div class="modal-overlay"><div class="modal-content"><span class="close-btn" data-close-modal>❌</span><h3>🔔 Réglages des notifications</h3><p class="text-muted text-small">Choisis exactement ce que White Wolf peut te notifier. Les notifications ne modifient jamais automatiquement ton Planning.</p><div style="display:grid;gap:2px">'+rows+'</div><div style="margin-top:14px"><label style="display:flex;justify-content:space-between;align-items:center"><span>🌙 Mode silencieux</span><input type="checkbox" id="notif-quiet" '+(ns.quietHours&&ns.quietHours.enabled?'checked':'')+'></label><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px"><input id="notif-quiet-start" type="time" value="'+((ns.quietHours&&ns.quietHours.start)||'22:00')+'"><input id="notif-quiet-end" type="time" value="'+((ns.quietHours&&ns.quietHours.end)||'07:00')+'"></div></div><div style="margin-top:14px;padding-top:12px;border-top:1px solid rgba(255,255,255,.08)"><strong>📡 Web Push (appareil fermé)</strong><div class="text-muted text-small" style="margin:6px 0 8px">بعد نشر Push Gateway، ضع عنوانه ومفتاح VAPID العام هنا. لا تضع المفتاح الخاص أو ADMIN_TOKEN.</div><input id="notif-push-endpoint" type="url" placeholder="https://votre-gateway.example/subscribe" value="'+((ns.push&&ns.push.endpoint)||'')+'" style="margin-bottom:8px"><input id="notif-push-vapid" type="text" placeholder="VAPID public key" value="'+((ns.push&&ns.push.vapidPublicKey)||'')+'"></div><div class="modal-actions"><button class="btn-outline" data-close-modal>Fermer</button><button class="btn-primary" data-save-notif-settings>💾 Enregistrer</button></div></div></div>'}
...
function attachAppEvents(){
  ...
  document.querySelectorAll('[data-save-notif-settings]').forEach(function(el){el.onclick=function(){if(window.WWNotifications){var cats={};document.querySelectorAll('[data-notif-cat]').forEach(function(x){cats[x.dataset.notifCat]=x.checked});WWNotifications.updateSettings({categories:cats,quietHours:{enabled:!!document.getElementById('notif-quiet')?.checked,start:(document.getElementById('notif-quiet-start')||{}).value||'22:00',end:(document.getElementById('notif-quiet-end')||{}).value||'07:00'},push:{enabled:true,endpoint:(document.getElementById('notif-push-endpoint')||{}).value||'',vapidPublicKey:(document.getElementById('notif-push-vapid')||{}).value||''} });}state.modal=null;showToast('🔔 Réglages enregistrés');render()}});
