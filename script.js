...
function wwSetSubjectScope(subjectId,active){wwEnsureStudyScope();var topics=state.topics.filter(function(t){return t.subject_id===subjectId});state.studyScope.subjects[subjectId]=!!active;topics.forEach(function(t){state.studyScope.topics[t.id]=!!active});saveState();render()}
function wwSetTopicScope(topicId,active){wwEnsureStudyScope();var t=state.topics.find(function(x){return x.id===topicId});if(!t)return;state.studyScope.topics[topicId]=!!active;var siblings=state.topics.filter(function(x){return x.subject_id===t.subject_id});var anyActive=siblings.some(function(x){return state.studyScope.topics[x.id]===true});state.studyScope.subjects[t.subject_id]=anyActive;saveState();render()}
