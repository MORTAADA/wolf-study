...
(function(global){
  ...
  function create(ctx){
    ...
    var tasks={
      ...
      save:function(){var text=document.getElementById('task-text').value,date=document.getElementById('task-date').value,time=document.getElementById('task-time').value,priority=document.getElementById('task-priority').value;if(!text){alert('Écrire une tâche');return}state.tasks.push({id:ctx.generateId(),text:text,priority:priority,date:date,time:time,isDone:false});state.modal=null;finish()},
      done:function(id){var t=state.tasks.find(function(x){return x.id===id});if(t){t.isDone=true;state.xp+=5;finish()}},
