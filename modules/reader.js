...
(function(){
  ...
  function ingestDocument(result){
    ...
    try{
      ...
      if(global.state){
        global.state.documentIntelligence.lastExtracted={name:R.meta.name||R.currentFile.name||'Document',text:result.text,extractedAt:new Date().toISOString(),method:result.method||'local',resourceId:R.meta.resourceId||null,topicId:R.meta.topicId||null};
    ...
    if(type==="application/pdf" || ext==="pdf"){
      ...
      ocrBtn.addEventListener("click",async function(){
        ...
        try{
          ...
          if(window.state){window.state.documentIntelligence.lastExtracted={name:file.name||"PDF",text:result.text||"",extractedAt:new Date().toISOString(),method:result.method};if(window.saveState)window.saveState();}
      ...
      extractBtn.addEventListener("click",async function(){
        ...
        try{
          ...
          if(window.state){window.state.documentIntelligence.lastExtracted={name:file.name||"PDF",text:result.text||"",extractedAt:result.extractedAt};if(window.saveState)window.saveState();}
