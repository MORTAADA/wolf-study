...
(function(global){
  ...
  function create(ctx){
    ...
    var tasks={
      ...
      save:function(){var text=(document.getElementById('task-text')||{}).value.trim(),date=(document.getElementById('task-date')||{}).value||ctx.wwTodayISO(),time=(document.getElementById('task-time')||{}).value||'',deadlineTime=(document.getElementById('task-deadline-time')||{}).value||'',estimatedMinutes=Math.max(5,Math.min(1440,Number((document.getElementById('task-estimated-min')||{}).value)||30)),priority=(document.getElementById('task-priority')||{}).value||'Moyenne',subjectId=(document.getElementById('task-subject')||{}).value||'',topicId=(document.getElementById('task-topic')||{}).value||'';if(!text){alert('Écrire une tâche');return}if(topicId&&subjectId&&ctx.state.topics){var topic=ctx.state.topics.find(function(x){return x.id===topicId});if(topic&&topic.subject_id!==subjectId){alert('Le chapitre ne correspond pas à la matière.');return}}if(topicId&&ctx.state.topics){var linked=ctx.state.topics.find(function(x){return x.id===topicId});if(linked)subjectId=linked.subject_id||subjectId}if(time&&deadlineTime){var a=time.split(':').map(Number),b=deadlineTime.split(':').map(Number),am=(a[0]||0)*60+(a[1]||0),bm=(b[0]||0)*60+(b[1]||0);if(bm<=am){alert('L’heure limite doit être après l’heure de début.');return}if(!Number((document.getElementById('task-estimated-min')||{}).value))estimatedMinutes=bm-am}state.tasks=Array.isArray(state.tasks)?state.tasks:[];var task={id:ctx.generateId(),text:text,title:text,priority:priority,date:date,time:time,deadlineTime:deadlineTime,estimatedMinutes:estimatedMinutes,subjectId:subjectId||null,subject_id:subjectId||null,topicId:topicId||null,topic_id:topicId||null,isDone:false,status:'todo',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};state.tasks.push(task);if(window.WWAcademicEvents)WWAcademicEvents.emit('TASK_CREATED',{taskId:task.id,date:date,time:time,deadlineTime:deadlineTime,estimatedMinutes:estimatedMinutes,priority:priority,subjectId:task.subjectId,topicId:task.topicId});state.modal=null;finish()},
